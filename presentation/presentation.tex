...
We had direct our focus on bandit algorithms combining Gaussian Process with Upper Confidence Bound and Thompson Sampling algorithm.
For each different scenario the performance of the learners based on these algorithm are evaluated against the Clayrvoiant and the Stupid Learner.
...
\begin{itemize}[leftmargin=*, label={$\circ$}]
    ...
    \item Each $alpha$-function is defined by the values: \textbf{budget}, \textbf{upper bound} and \textbf{maximum useful budget}
\end{itemize}
...
We modelled the $Alpha$-functions, which compute the expected value of interaction on a product given a fixed budget, as exponential functions.
In particular thier upper bound represents the maximum expected number of interaction possible while the maximum useful budget is amount of budget after which its increading would not lead to a ratio increase.


\todo{do we want a new slide with a img? }
%\begin{figure}[⟨r⟩]
%\includegraphics[height=3.8cm]{img/Graphs/.png}
%\end{figure}
...
Added slide tell me if is enough or need more introduction.}
...
We assume now that the binary features of the users can not be observed and therefore data are aggregated.
...
Since the feature of the users are \textbf{not osservable}, the algorithm receive all the interactions minus the class data.
