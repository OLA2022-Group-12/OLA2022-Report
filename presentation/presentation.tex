...
In order to make a more universal modus operandi, the Generic Learner Class is a costum class in python in whitch all our learner belong.
This class is for interactive agents capable of learning the budget distribution for a set of subcampaigns from the specific environment in which they exist.
...
\frametitle{General Approach}
\framesubtitle{Learn and Predict}

In particular this class is characterized by the actions of learning from the interactions and predicting best budget distribution.
Each learner will have different set of information given to him from the masked enviroment accordingly with the project;
and potencially could have different algorithm to learn and predict.
...
Both Thompson Sampling and Upper Confidence Bound are the bandit algorithm, combined with Gaussian Process, that the learners use.
\todo{do we want here an example of the regret of 1 the learners as the time horrizion become larget to show the logartmic increase of the regret? and to show linearly increase with cartinality of arm?}
...
For each different learner we will show our practical result by comparing the reward of each algorithm against the one given in the same enviroment setting from Clayrvoiant learner and the "stupid" one.
This helps to fix a upper bound and a lower bound to the performance of our learner.
