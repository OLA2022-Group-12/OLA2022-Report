...
\todo{insert TS and UCB1 formulations}
...
\todo{briefly explain what is the stupid learner}
\todo{this two slides repeat a lot without adding much, we need to move this part in the Introduction and merge this with the slide that already talks about this}
\end{frame}
...
We now assume that the binary features of the users cannot be observed and therefore user data is aggregated.

Since the features of the users are \textbf{not observable}, the algorithm receives all the interactions minus the class data.

\todo{are you sure that this is right? We cannot observe the class parameters, not the class itself. What is unknown here is the shape of the alpha function!}
